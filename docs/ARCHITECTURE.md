...
`PromptRefreshCoordinator` computes every slot from now through 12 calendar days out (`PrayerTimeline.slots(from:through:)`), drops whatever's already passed today, and hands the result to `PromptScheduler` — 12 × 5 = 60, under the 64 cap even with nothing disabled. `BackgroundRefreshScheduling`/`SystemBackgroundRefreshScheduler` wrap only the "ask for the next wake-up" half of `BGTaskScheduler` (`BGAppRefreshTaskRequest`/`submit`); registering the launch handler itself is SwiftUI's `.backgroundTask(.appRefresh(_:))` modifier, wired on `RukiApp` (T2). `BackgroundRefresh.refreshAndScheduleNext` is the one call that handler makes — `RukiApp` also calls it directly on launch (`.task`) and on returning to the foreground (`.onChange(of: scenePhase)`), so app-launch and foreground are covered. **Settings-change and pause-change triggers are still missing** — those hooks belong to RUKI-036 and RUKI-034, not yet built; RUKI-015 stays `partial` for that reason.
...
- `TodayView`'s check-in button and pause entry point both open `ComingSoonScreen` — no real capture flow (RUKI-019) or pause flow (RUKI-034) exists yet.
- Background-refresh scheduling now runs on app launch and foreground (`RukiApp`, T2) but still has no settings-change or pause-change trigger (RUKI-036, RUKI-034 respectively).
