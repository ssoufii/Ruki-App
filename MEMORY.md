...
**Product question left open:** a solo user who later creates an account starts with an *empty* history under it (their solo history stays under "signed out"). Adopting it on first sign-up is a real choice (a different person on a shared phone would inherit it) — not decided.
