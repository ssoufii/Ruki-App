...
| ~~D47~~ | ~~Dev-only `RUKI_UNLOCK_ALL=1` disables the D9 lock on the local server.** Default stays locked. | Local history is per *device*, not per account, so on one simulator a second account cannot check in for a prayer the first already did — it could never unlock the feed. D9 itself is unchanged. | History becomes per-account, or two-device testing | **Superseded by D48** — the flag still exists but is no longer needed for one-device testing. |
| **D48** | **On-device history is per account.** Signed-out use keeps the original store (nothing orphaned); each account gets its own SwiftData file and its own "started using Ruki" moment (set on first use on the device). Today rebuilds when the login changes. "Delete my data" and photo-expiry purge cover every account's store on the device. | User rejected the one-device limitation: two accounts on one phone must not share check-ins (a friend's Fajr tick was showing as mine, so I couldn't check in for it and the feed stayed locked). | Adoption of solo history on first sign-up (see below) |
| **D49** | **"I prayed" (marked after the window closed) is shared with the circle as a photo-less post; "I didn't" stays on the device.** The post lives until the next prayer begins *from the moment of the tick* (the usual "next prayer after the window" would already be past, since the mark only exists once the window is closed). A photo-less post is never locked by D9 (nothing to hide). Server: photos optional (schema migration). | User decision (2026-09-21): someone who forgot to post should still show up for friends. Affects PRD §7 (marks were fully private) — **PRD not yet updated**. Not an absence broadcast (RDP-2): only the positive answer is ever sent. Honesty cost: no photo = a bare claim; the app never verified prayers anyway. | Users treat the tick as a way to skip the photo (OQ-7-adjacent) |
...
**Session 10 addendum 3 — per-account history (D48) + names:** `HistoryOwner`/`HistoryStores`; `AppEnvironment.historyStore` follows the login; `RootView` rebuilds Today by owner key; `HistoryStore.trackingStart` (per account). Typed names normalise to usernames ("Sumeya Farah" → `sumeya_farah`) on server and client, with a live preview.
**Tested:** new `HistoryStoresTests` (separation, same-instance, per-account start date persisting across relaunch, allStores); server 8/8; unit suite green; live two-user story re-run green.
**NOT tested:** the switch itself on screen (log out → log in as someone else → fresh Today); the Settings→login-while-skipped path; a real upgrade of the existing simulator (guest keeps the old store, which still holds the earlier mixed test check-ins — use Settings → Delete my data for a clean slate). Expired photos in an inactive account's store are purged on the next refresh, not the instant they expire.
**Decided (user, 2026-09-20): strict separation, no adoption.** Every account's data belongs to that account alone; a solo history stays under "signed out" and is never copied into an account. Logging out and back in restores the account's own history (tested, including across an on-disk relaunch).

**Session 10 addendum 4 — debug clock ratchet:** my "next occurrence" jump (earlier today) measured from the already-jumped clock, so each jump walked the date forward a day; two accounts ended on different slot IDs (`…-09-23.dhuhr` vs `…-09-26.dhuhr`) and never unlocked each other. Fixed: jumps are measured from `OffsetClock.realNow()`; an account's tracking start uses real time and is clamped if a jumped value was saved. Tests added (no-ratchet, clamp). **Not tested on screen.** Old server rows with drifted slot IDs stay in the feed until they expire — reset `server/data` for a clean test.
**Rule for me:** debug tooling that changes time needs a two-account check, not only single-account unit tests.

**Session 10 addendum 5 — debug clock, second fix:** "next occurrence measured from real time" still split days once real time passed a prayer (Fajr rolled to tomorrow, Dhuhr/Asr stayed today), so Today showed a different day and check-ins *looked* erased (server showed Fajr on 09-27, Dhuhr/Asr on 09-28 from the old ratcheting build). Now every scenario is **tomorrow's** prayer, measured from an anchor pinned at the first jump and cleared by "Reset clock" (`OffsetClock.testAnchor()`). Check-in records are never deleted by time passing — only photo bytes are purged after expiry (D26/D37). Test added: all scenarios share one day at any time of day. **Not tested on screen.** Data from before this build is on scattered days — reset `server/data` and erase the simulator.

**Session 10 addendum 6 — UI restructure, profile, shared "I prayed":** bottom tabs (Today / Friends / Circle / Profile); Today redesigned (centred hero card for the open prayer, five equal prayer circles, "n of 5 today"); new Profile tab (large private streak, lifetime, 30-day rate, circle size, History, Settings); login header now "Ruki"; D49 above. Mark sheet copy rewritten so it no longer claims "never leaves your device" for "I prayed" (it would have been false).
**Tested:** unit suite green (new `MarkSharingTests`: prayed → one photo-less upload with future expiry; didn't → nothing sent; signed out → local); server 9/9 incl. old-schema migration; live two-user story re-run. **NOT tested:** any of the new screens rendered (tabs, Today hero/strip, Profile, mark sheet, photo-less feed row); dark mode; small phones (the 5-across strip and 3 stat tiles are the layout risks); VoiceOver.
**PRD/RDP follow-up:** update PRD §7 for D49; run `/values-check` on the shared mark before anyone but the builder uses it.
